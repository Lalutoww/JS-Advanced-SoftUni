function solve() {
    ...
    archive.nextElementSibling.addEventListener(
        'click',
    ...
    function addMovieOnScreen(e) {
        ...
        Array.from(displayMovie.querySelectorAll('button')).forEach((btn) =>
        ...
        function addToArchive(btn) {
            ...
            if (Number(ticketsSold.value) || ticketsSold.value === '0') {
                archive.innerHTML += `<li>
                        <span>${
                            btn.target.parentNode.parentNode.children[0]
                                .textContent
                        }</span>
                        <strong>Total amount: ${(
                ...
                Array.from(archive.querySelectorAll('button')).forEach((btn) =>
