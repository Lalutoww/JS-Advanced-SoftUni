class Company {
    ...
    bestSalaryDepart() {
        return this.departments.sort(
            (a, b) =>
                ...
                this.getSalariesSum(a) / a.salaries.length // sort by dept avg salary
        )[0]; // [0] means highest salary
    }
    ...
    bestDepartment() {
        ...
        const combinedEmployees = best.employees.reduce((a, v, i) => {
            a[i] = []; //Creates a matrix for every employee
            a[i].push(v, best.salaries[i], best.positions[i]);
            return a;
        }, []);

        const sorted = combinedEmployees.sort((a, b) => {
            return b[1] - a[1] === 0 ? a[0].localeCompare(b[0]) : b[1] - a[1]; //if salaries are equal, sort by name
        });
